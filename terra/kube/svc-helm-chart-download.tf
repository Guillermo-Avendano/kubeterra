...
resource "null_resource" "download_eventanalytics_helm_chart" {
  ...
  provisioner "local-exec" {
    command = <<EOT
set -e
if [ ! -f "${path.root}/../helm_charts/${var.var_eventanalytics_chart_file}" ]; then
  ...
  curl -X GET "https://wal-artifactory.rocketsoftware.com/artifactory/mobf-helm-dev-wal/${var.var_eventanalytics_chart_file}" \
      ...
      -o "${path.root}/../helm_charts/${var.var_eventanalytics_chart_file}"
else
...
resource "null_resource" "download_mobius_server_helm_chart" {
  ...
  provisioner "local-exec" {
    command = <<EOT
set -e
if [ ! -f "${path.root}/../helm_charts/${var.var_mobiusserver_chart_file}" ]; then
  ...
  curl -X GET "https://wal-artifactory.rocketsoftware.com/artifactory/mobf-helm-dev-wal/${var.var_mobiusserver_chart_file}" \
       -u "${var.var_docker_username}:${var.var_docker_password}" \
      -o "${path.root}/../helm_charts/${var.var_mobiusserver_chart_file}"
else
...
resource "null_resource" "download_smart_chat_helm_chart" {
  ...
  provisioner "local-exec" {
    command = <<EOT
set -e
if [ ! -f "${path.root}/../helm_charts/${var.var_smart_chat_chart_file}" ]; then
  ...
  curl -X GET "https://wal-artifactory.rocketsoftware.com/artifactory/bicy-helm-release-wal/smart-chat/${var.var_smart_chat_chart_file}" \
       -u "${var.var_docker_username}:${var.var_docker_password}" \
      -o "${path.root}/../helm_charts/${var.var_smart_chat_chart_file}"
else
...
resource "null_resource" "download_smart_chat_indexing_proxy_helm_chart" {
  ...
  provisioner "local-exec" {
    command = <<EOT
set -e
if [ ! -f "${path.root}/../helm_charts/${var.var_smart_chat_indexing_proxy_chart_file}" ]; then
  ...
  curl -X GET "https://wal-artifactory.rocketsoftware.com/artifactory/bicy-helm-release-wal/smart-chat-indexing-proxy/${var.var_smart_chat_indexing_proxy_chart_file}" \
       -u "${var.var_docker_username}:${var.var_docker_password}" \
      -o "${path.root}/../helm_charts/${var.var_smart_chat_indexing_proxy_chart_file}"
else
...
resource "null_resource" "download_mobius_view_helm_chart" {
  ...
  provisioner "local-exec" {
    command = <<EOT
set -e
if [ ! -f "${path.root}/../helm_charts/${var.var_mobiusview_chart_file}" ]; then
  ...
  curl -X GET "https://wal-artifactory.rocketsoftware.com/artifactory/mobf-helm-dev-wal/${var.var_mobiusview_chart_file}" \
       -u "${var.var_docker_username}:${var.var_docker_password}" \
      -o "${path.root}/../helm_charts/${var.var_mobiusview_chart_file}"
else
...
resource "null_resource" "download_appmanager_helm_chart" {

  count = var.var_deploy_appmanager ? 1 : 0
  provisioner "local-exec" {
    command = <<EOT
set -e
if [ ! -f "${path.root}/../helm_charts/${var.var_appmanager_chart_file}" ]; then
  echo "Downloading AppManager Helm chart..."
  curl -X GET "https://wal-artifactory.rocketsoftware.com/artifactory/mobf-helm-dev-wal/${var.var_appmanager_chart_file}" \
       -u "${var.var_docker_username}:${var.var_docker_password}" \
      -o "${path.root}/../helm_charts/${var.var_appmanager_chart_file}"
else
  echo "AppManager chart already exists. Skipping download."
fi
EOT
  }
...
resource "null_resource" "download_studio_helm_chart" {

  count = var.var_deploy_studio ? 1 : 0
  provisioner "local-exec" {
    command = <<EOT
set -e
if [ ! -f "${path.root}/../helm_charts/${var.var_studio_chart_file}" ]; then
  echo "Downloading Studio Helm chart..."
  curl -X GET "https://wal-artifactory.rocketsoftware.com/artifactory/mobf-helm-dev-wal/${var.var_studio_chart_file}" \
       -u "${var.var_docker_username}:${var.var_docker_password}" \
      -o "${path.root}/../helm_charts/${var.var_studio_chart_file}"
else
  echo "Studio chart already exists. Skipping download."
fi
EOT
  }
...
resource "null_resource" "download_processengine_helm_chart" {

  count = var.var_deploy_processengine ? 1 : 0
  provisioner "local-exec" {
    command = <<EOT
set -e
if [ ! -f "${path.root}/../helm_charts/${var.var_processengine_chart_file}" ]; then
  echo "Downloading ProcessEngine Helm chart..."
  curl -X GET "https://wal-artifactory.rocketsoftware.com/artifactory/mobf-helm-dev-wal/${var.var_processengine_chart_file}" \
       -u "${var.var_docker_username}:${var.var_docker_password}" \
      -o "${path.root}/../helm_charts/${var.var_processengine_chart_file}"
else
  echo "ProcessEngine chart already exists. Skipping download."
fi
EOT
  }
